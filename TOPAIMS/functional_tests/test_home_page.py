...
class HomePageTest(FunctionalTest):
	...
	#- DELIVERIES -#
		...
		today_deliveries_panel = self.browser.find_element_by_id('parent_deliveries_panel').find_element_by_id('today_deliveries_panel')
		...
		self.assertIn('today fname 1', all_deliveries_panel.get_attribute('innerHTML'))
		self.assertIn('today fname 2', all_deliveries_panel.get_attribute('innerHTML'))
		self.assertIn('today fname 3', all_deliveries_panel.get_attribute('innerHTML'))
		self.assertIn('thisweek fname 1', all_deliveries_panel.get_attribute('innerHTML'))
		...
		self.wait_for(lambda: self.assertNotIn('today fname 1', today_deliveries_panel))
		...
		self.click(base_element='today_deliveries_panel', element=f'reject_delivery_button{today_2_object.pk}')
		...
		""" marek fills in a note and the reschedule date and hits submit """
		...
		# Marek checks the job view and sees the item as 'en-route' in the delivery panel with the new date added
		self.browser.get(self.live_server_url+reverse('job', kwargs={'job_id':'200ParkAvenue'}))
		self.wait_until_visible('site_management_panel')
		self.click(base_element='site_management_panel', element='en_route_panel_toggle')
		en_route_panel = self.browser.find_element_by_id('site_management_panel').find_element_by_id('en_route_panel')

		self.wait_for(lambda: self.assertIn(f'today fname 2 | status - ORDERED | delivery: {today_2_object.delivery_date}')) # may render in the FT differently to the browser, edit as needed
		...
		self.wait_for(lambda: self.assertIn('note marek typed in when rejecting item 2. || rescheduled for delivery on x date', notes_panel.get_attribute('innerHTML'))) # edit to match browser
		...
		self.click_element(today_deliveries_panel_toggle)
		self.click(base_element='today_deliveries_panel', element=f'reject_delivery_button{today_3_object.pk}')
		...
		""" fills in a note and does not reschedule in the rejection modal form and hits submit"""
		...
		self.assertNotIn('today fname 3', today_deliveries_panel)

		self.click_element(this_week_deliveries_panel_toggle)
		self.wait_until_visible('this_week_deliveries_panel')
		self.assertNotIn('today fname 3', this_week_deliveries_panel)

		self.click_element(all_deliveries_panel_toggle)
		self.wait_until_visible('all_deliveries_panel')
		self.assertNotIn('today fname 3', all_deliveries_panel)
		...
		self.wait_for(lambda: self.assertIn('note marek typed in when rejecting item 3. || NOT RESCHEDULED', notes_panel.get_attribute('innerHTML'))) # edit to match browser
...
#- Notes -#
		...
		# Now Marek is looking at the notes section and sees two tabs, 'admin' and 'all'
		...
		self.wait_for(lambda: self.assertIn('test admin note TITLE', admin_notes_panel))
		self.wait_for(lambda: self.assertIn('test admin note TEXT', admin_notes_panel))
		...
		form_job.select_by_value('HellsKitchen')
		...
		self.assertIn('item C fullname', all_deliveries_panel.get_attribute("innerHTML"))
		self.assertIn('item A fullname', all_deliveries_panel.get_attribute("innerHTML"))