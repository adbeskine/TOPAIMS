...
class ShoppingListPageTest(FunctionalTest):
  ...
  #--------------------------------------------------------------#
	...
	def test_shopping_list_page(self):
  		...
  		self.click(base_element='shopping_list_item_form', element='shopping_list_form_submit_button')
  		...
  		self.assertIn(new_shopping_list_item.get_attribute("innerHTML"), 'shopping list item 1')
  		self.assertIn(new_shopping_list_item.get_attribute("innerHTML"), '1')
  		self.assertIn(new_shopping_list_item.get_attribute("innerHTML"), '200 Park Avenue')
  		...
  		self.wait_for(lambda: self.assertNotIn(self.browser.source, 'id="shopping_list_tems_1"'))
  		# an alert says 'x acquired'
  		self.wait_for(lambda: self.assertIn(self.browser.source, 'shopping list item 1 acquired'))
